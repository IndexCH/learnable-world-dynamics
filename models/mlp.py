...
class DynamicsMLP(nn.Module):
    """
    Phase 1 ：嵌入辛欧拉积分器 (Symplectic Euler Integrator)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        #x: [batch_size, 6] -> [pos_x, pos_y, vel_x, vel_y, fx, fy]
        pos = x[:, 0:2]
        vel = x[:, 2:4]
        force = x[:, 4:6]
        
        #剥离绝对坐标，只把速度和受力喂给神经网络
        kinematic_features = torch.cat([vel, force], dim=1)
        
        #网络只负责一件事：预测速度的变化量 (Delta Velocity)
